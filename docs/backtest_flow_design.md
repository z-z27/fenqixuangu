...
- 收盘价重新站回当日 VWAP。
- 不能追高；没有回踩则不成交。
- 默认成交价按确认 K 线收盘价 `confirmation_close` 计；`zone_max` 仅保留给旧结果对照，属于更乐观的成交假设。
- 确认成交的 5 分钟 K 线不计入买后最高价、最低价和止盈止损判断，买后路径从下一根 K 线开始。
- 同一根 5 分钟 K 线同时触及止盈和止损时无法判断盘中先后，主口径按止损优先，并标记 `same_bar_ambiguous=True`。
...
回测收益仍是研究口径。除非报告明确给出净收益字段，否则不包含滑点、手续费、排队成交和流动性冲击。
