...
def build_markdown_report(frame: pd.DataFrame, trade_date: str) -> str:
    ...
    lines.append("| 总分 | 趋势保持×0.35 + 图形×0.25 + 活跃冷却×0.20 + 入场宽度×0.10 + 题材×0.10（承接当前权重 0） |")
    lines.append("| 图形 | 趋势结构、均线排列、K线位置；当前作为总分分量 |")
    lines.append("| 活跃 | 量能放大、振幅、分时VWAP争夺；active_cooling 是当前总分分量 |")
    lines.append("| 承接 | 低点支撑、VWAP站位、关键位收回；保留作诊断/旧流程兼容，当前总分权重为 0 |")
